...
class report_account_aged_partner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        columns = [{}]
        columns += [
            {'name': v, 'class': 'number', 'style': 'white-space:nowrap;text-align:center;'}
            for v in [_("Journal"), _("Reference"), _("Not due on: %s") % format_date(self.env, options['date']['date'])]
        ]

        columns += [
            {'name': v, 'class': 'number', 'style': 'white-space:nowrap;'}
            for v in [_("1 - 15"), _("16 - 30"), _("31 - 45"), _("46 - 60"), _("61 - 90"), _(">90"), _("Total")]
        ]
        return columns

    def _get_templates(self):
        templates = super(report_account_aged_partner, self)._get_templates()
        templates['main_template'] = 'gts_account_report.template_aged_partner_balance_report'
        try:
            self.env['ir.ui.view'].get_view_id('gts_account_report.template_aged_partner_balance_line_report')
            templates['line_template'] = 'gts_account_report.template_aged_partner_balance_line_report'
        except ValueError:
            pass
        return templates
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results, total, amls = self.env['report.account.report_agedpartnerbalance'].with_context(include_nullified_amount=True)._get_partner_move_lines(account_types, self._context['date_to'], 'posted', 15)
        for values in results:
            ...
            if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
                for line in amls[values['partner_id']]:
                    ...
                    line_date = aml.invoice_id.date_invoice
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in [aml.journal_id.code, aml.invoice_id.number]] +\
...
class report_account_aged_receivable(models.AbstractModel):
    ...
    def _get_report_name(self):
        return _("Aging Schedule Account Receivable")

    def _get_templates(self):
        templates = super(report_account_aged_receivable, self)._get_templates()
        templates['line_template'] = 'gts_account_report.line_template_aged_receivable_report'
        return templates
...
class report_account_aged_payable(models.AbstractModel):
    ...
    def _get_report_name(self):
        return _("Aging Schedule Account Payable")

    def _get_templates(self):
        templates = super(report_account_aged_payable, self)._get_templates()
        templates['line_template'] = 'gts_account_report.line_template_aged_payable_report'
        return templates

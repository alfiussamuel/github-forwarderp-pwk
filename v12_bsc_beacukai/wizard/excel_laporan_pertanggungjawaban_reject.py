...
class ReportBarangjadi(models.AbstractModel):
    _name = 'report.v12_bsc_beacukai.report_reject'
    ...
    def render_html(self, docids, data=None):
        ...
        location_id = int(self.env['ir.config_parameter'].sudo().get_param('location_reject'))
        ...
        return self.env['report'].render('v12_bsc_beacukai.report_reject', docargs)

class ExcelLaporanPertanggungjawabanReject(models.TransientModel):
    _name = 'excel.laporan.pertanggungjawaban.reject'
    ...
    def open_table(self):
        ...
        #         'type': 'ir.actions.act_window',
        #     }
        loc = self.env['ir.config_parameter'].sudo().get_param('location_reject')
    ...
    def get_move_line(self,id_product):
        ...
        move_line_ids = self.env['stock.move'].search([
            ('date', '<=', self.date_to),                
            ('state','=','done'),
            submission_no ('location_id','=',location_id.id), ('location_dest_id','=',location_id.id)
            ])
    ...
    def generate_preview(self):
        ...
        loc = int(self.env['ir.config_parameter'].sudo().get_param('location_reject'))
        location_id = self.env['stock.location'].browse(loc)
        ...
        for res in self:
            ...
            if moveline_ids:          
                ...
                current_report_ids = self.env['laporan.mutasi.reject'].search([('report_name', '=', report_name)])
                ...
                for product in moveline_ids.mapped('product_id'):              
                    mutasi_id = self.env['laporan.mutasi.reject'].create({
                                                                    'name': report_name,
                    ...
                    if product.active:                
                        for move_line in moveline_ids:
                            ...
                            if move_line.location_id.id == location_id.id and move_line.product_id.id == product.id and move_line.date_expected < self.date_from:                                                    
                                saldo_awal -= move_line.product_uom_qty
                                move_line.write({'mutasi_reject_id': mutasi_id.id, 'tipe_mutasi':'Saldo Awal'})
                                ...
                                if move_line.location_dest_id.usage == "inventory":                                
                                    penyesuaian -= move_line.product_uom_qty
                                    move_line.write({'mutasi_reject_id': mutasi_id.id, 'tipe_mutasi':'Penyesuaian'})                                
                                else:                                
                                    pengeluaran += move_line.product_uom_qty    
                                    move_line.write({'mutasi_reject_id': mutasi_id.id, 'tipe_mutasi':'Pengeluaran'})                            
                            elif move_line.location_dest_id.id == location_id.id and move_line.product_id.id == product.id and move_line.date_expected >= self.date_from:                        
                                if move_line.location_id.usage == "inventory":                                
                                    penyesuaian += move_line.product_uom_qty    
                                    move_line.write({'mutasi_reject_id': mutasi_id.id, 'tipe_mutasi':'Penyesuaian'})                            
                                    ...
                                    pemasukan += move_line.product_uom_qty      
                                    move_line.write({'mutasi_reject_id': mutasi_id.id, 'tipe_mutasi':'Pemasukan'})                          
        ...
        if mutasi_ids:
            ...
            action = self.env['ir.model.data'].xmlid_to_object('v12_bsc_beacukai.action_laporan_mutasi_reject')
            if not action:
                action = {
                    'view_type': 'form',
                    'view_mode': 'tree',
                    'res_model': 'laporan.mutasi.reject',
                    'type': 'ir.actions.act_window',
                }
            else:
                action = action[0].read()[0]
            ...
            action['name'] = _('Laporan Mutasi Reject Jadi Per ' + str(self.date_from) + ' - ' + str(self.date_to))        
            action['context'] = ctx
            return action
        else:
    ...
    def _print_report(self, data):
        ...
        return self.env['report'].get_action(self, 'v12_bsc_beacukai.report_reject', data=data)

    @api.multi
    def generate_report(self):
        filename = 'Laporan Pertanggungjawaban Mutasi Barang Reject dan Scrap.xlsx'
        ...
        worksheet1.merge_range('A2:L2', 'Laporan Pertanggungjawaban Mutasi Barang Reject dan Scrap', left_title)
        ...
        i = 0
        loc = int(self.env['ir.config_parameter'].sudo().get_param('location_reject', default=''))
        location_id = self.env['stock.location'].browse(loc)
        ...
        form_res = ir_model_data.get_object_reference(
            'v12_bsc_beacukai', 'excel_laporan_pertanggungjawaban_reject_form')
        form_id = form_res and form_res[1] or False
        return {
            'name': ('Download XLS'),
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'excel.laporan.pertanggungjawaban.reject',
            'res_id': self.id,
            'view_id': False,
            'views': [(form_id, 'form')],
            'type': 'ir.actions.act_window',

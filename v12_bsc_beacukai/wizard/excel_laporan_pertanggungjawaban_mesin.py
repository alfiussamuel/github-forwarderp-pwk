...
class ReportMesin(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        location_id = int(self.env['ir.config_parameter'].sudo().get_param('location_mesindanperalatan'))
...
class ExcelLaporanPertanggungjawabanMesin(models.TransientModel):
    ...
    def open_table(self):
        ...
        #         'type': 'ir.actions.act_window',
        #     }
        loc = self.env['ir.config_parameter'].sudo().get_param('location_mesindanperalatan')
        ...
        action['name'] = _('Laporan mesin dan peralatan')
        action['context'] = ctx
        return action

    def get_move_line(self,id_product):
        product = self.env['product.product'].browse(id_product)
        loc = int(self.env['ir.config_parameter'].sudo().get_param('location_mesindanperalatan'))
        location_id = self.env['stock.location'].browse(loc)
        move_line_ids = self.env['stock.move'].search([
            ('date', '<=', self.date_to),                
            ('state','=','done'),
    ...
    def generate_preview(self):
        ...
        for res in self:
            ...
            if moveline_ids:          
                ...
                current_report_ids = self.env['laporan.mutasi.mesin'].search([('report_name', '=', report_name)])
                ...
                for product in moveline_ids.mapped('product_id'):              
                    mutasi_id = self.env['laporan.mutasi.mesin'].create({
                                                                    'name': report_name,
                    ...
                    if product.active:                
                        for move_line in moveline_ids:
                            if move_line.location_id.id == location_id.id and move_line.product_id.id == product.id and move_line.date_expected < self.date_from:                                                    
                                saldo_awal -= move_line.product_uom_qty
                                move_line.write({'mutasi_mesin_id': mutasi_id.id, 'tipe_mutasi':'Saldo Awal'})
                                ...
                                if move_line.location_dest_id.usage == "inventory":                                
                                    penyesuaian -= move_line.product_uom_qty
                                    move_line.write({'mutasi_mesin_id': mutasi_id.id, 'tipe_mutasi':'Penyesuaian'})                                
                                else:                                
                                    pengeluaran += move_line.product_uom_qty    
                                    move_line.write({'mutasi_mesin_id': mutasi_id.id, 'tipe_mutasi':'Pengeluaran'})                            
                            elif move_line.location_dest_id.id == location_id.id and move_line.product_id.id == product.id and move_line.date_expected >= self.date_from:                        
                                if move_line.location_id.usage == "inventory":                                
                                    penyesuaian += move_line.product_uom_qty    
                                    move_line.write({'mutasi_mesin_id': mutasi_id.id, 'tipe_mutasi':'Penyesuaian'})                            
                                    ...
                                    pemasukan += move_line.product_uom_qty      
                                    move_line.write({'mutasi_mesin_id': mutasi_id.id, 'tipe_mutasi':'Pemasukan'})                          
        ...
        if mutasi_ids:
            ...
            if not action:
                action = {
                    'view_type': 'form',
                    'view_mode': 'tree',
                    'res_model': 'laporan.mutasi.mesin',
                    'type': 'ir.actions.act_window',
                }
            else:
                action = action[0].read()[0]
            ...
            action['name'] = _('Laporan Mutasi Mesin Per ' + str(self.date_from) + ' - ' + str(self.date_to))        
            action['context'] = ctx
            return action
        else:
    ...
    def generate_report(self):
        filename = 'Laporan Pertanggungjawaban Mutasi Mesin dan Peralatan.xlsx'
        ...
        worksheet1.merge_range('A2:L2', 'Laporan Pertanggungjawaban Mutasi Mesin dan Peralatan', left_title)
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'excel.laporan.pertanggungjawaban.mesin',
            'res_id': self.id,
            'view_id': False,
            'views': [(form_id, 'form')],
            'type': 'ir.actions.act_window',

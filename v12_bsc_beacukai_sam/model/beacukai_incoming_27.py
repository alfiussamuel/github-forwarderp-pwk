...
class BeacukaiSuratKeputusan27(models.Model):
    _name = "beacukai.surat.keputusan.incoming.27"    

    reference = fields.Many2one('beacukai.incoming.27', 'Reference')
...
class BeacukaiContainer27(models.Model):
    _name = "beacukai.container.incoming.27"    
...
class BeacukaiKemasan27(models.Model):
    _name = "beacukai.kemasan.incoming.27"    
...
class BeacukaiincomingLine27(models.Model):
    _name = "beacukai.incoming.line.27"
...
class BeacukaiincomingBahanBakuImport27(models.Model):
    _name = "beacukai.incoming.bahan.baku.import.27"    
...
class BeacukaiincomingBahanBakuLocal27(models.Model):
    _name = "beacukai.incoming.bahan.baku.local.27"    
...
class Beacukaiincoming27(models.Model):
    _name = "beacukai.incoming.27"
    ...
    packing_list_date = fields.Char('Tgl Packing List')
    ...
    """ One to many """
    surat_keputusan_ids = fields.One2many('beacukai.surat.keputusan.incoming.27', 'reference', string='Surat Keputusan Bersama', copy=True)
    container_ids = fields.One2many('beacukai.container.incoming.27', 'reference', string='Kontainer', copy=True)
    kemasan_ids = fields.One2many('beacukai.kemasan.incoming.27', 'reference', string='Kemasan', copy=True)
    line_ids = fields.One2many('beacukai.incoming.line.27', 'reference', string='Detail Barang', copy=True)
    bahan_baku_import_ids = fields.One2many('beacukai.incoming.bahan.baku.import.27', 'reference', string='Bahan Baku Import', copy=True)
    bahan_baku_local_ids = fields.One2many('beacukai.incoming.bahan.baku.local.27', 'reference', string='Bahan Baku Local', copy=True)
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('beacukai.incoming.27') or 'New'
        return super(Beacukaiincoming27, self).create(vals)

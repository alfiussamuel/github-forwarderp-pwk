...
class BeacukaiSuratKeputusan261(models.Model):
    _name = "beacukai.surat.keputusan.261"    

    reference = fields.Many2one('beacukai.outgoing.261', 'Reference')
    name = fields.Char('Nomor Dokumen')
    jenis_dokumen = fields.Char('Jenis Dokumen')
    date = fields.Date('Tanggal')


class BeacukaiContainer261(models.Model):
    _name = "beacukai.container.261"    
...
class BeacukaiKemasan261(models.Model):
    _name = "beacukai.kemasan.261"    
...
class BeacukaiOutgoingLine261(models.Model):
    _name = "beacukai.outgoing.line.261"
    _inherit = ['beacukai.outgoing.line.mixin']

    reference = fields.Many2one('beacukai.outgoing.261', 'Reference')
    jenis_kemasan = fields.Many2one('beacukai.jenis.kemasan', 'Jenis Kemasan')
    bahan_baku_import_ids = fields.One2many('beacukai.outgoing.line.bahan.baku.import.261', 'reference', 
        string='Bahan Baku Import', copy=True)
    bahan_baku_local_ids = fields.One2many('beacukai.outgoing.line.bahan.baku.local.261', 'reference', 
...
class BeacukaiOutgoingLineBahanBakuImport261(models.Model):
    _name = "beacukai.outgoing.line.bahan.baku.import.261"

    reference = fields.Many2one('beacukai.outgoing.line.261', 'Reference')
...
class BeacukaiOutgoingLineBahanBakuLocal261(models.Model):
    _name = "beacukai.outgoing.line.bahan.baku.local.261"    
...
# class BeacukaiOutgoingBahanBakuImport261(models.Model):
#     _name = "beacukai.outgoing.bahan.baku.import.261"    
...
# class BeacukaiOutgoingBahanBakuLocal261(models.Model):
#     _name = "beacukai.outgoing.bahan.baku.local.261"    
...
class BeacukaiOutgoing261(models.Model):
    _name = "beacukai.outgoing.261"
    ...
    def _default_document_type_id(self):        
        document_type_id = self.env['beacukai.document.type'].search([('name', '=', '261')])
        if document_type_id:
    ...
    company_permission_date = fields.Date('Tgl Izin')
    ...
    """ One to many """
    surat_keputusan_ids = fields.One2many('beacukai.surat.keputusan.261', 'reference', string='Surat Keputusan Bersama', copy=True)
    container_ids = fields.One2many('beacukai.container.261', 'reference', string='Kontainer', copy=True)
    kemasan_ids = fields.One2many('beacukai.kemasan.261', 'reference', string='Kemasan', copy=True)
    line_ids = fields.One2many('beacukai.outgoing.line.261', 'reference', string='Detail Barang', copy=True)
    # bahan_baku_import_ids = fields.One2many('beacukai.outgoing.bahan.baku.import.261', 'reference', string='Bahan Baku Import', copy=True)
    # bahan_baku_local_ids = fields.One2many('beacukai.outgoing.bahan.baku.local.261', 'reference', string='Bahan Baku Local', copy=True)
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('beacukai.outgoing.261') or 'New'
        return super(BeacukaiOutgoing261, self).create(vals)

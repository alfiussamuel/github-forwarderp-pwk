...
class BeacukaiSuratKeputusan262(models.Model):
    _name = "beacukai.surat.keputusan.262"    

    reference = fields.Many2one('beacukai.incoming.262', 'Reference')
    name = fields.Char('Nomor Dokumen')    
    jenis_dokumen = fields.Char('Jenis Dokumen')
    date = fields.Date('Tanggal')


class BeacukaiContainer262(models.Model):
    _name = "beacukai.container.262"    
...
class BeacukaiKemasan262(models.Model):
    _name = "beacukai.kemasan.262"    
...
class BeacukaiincomingLine262(models.Model):
    _name = "beacukai.incoming.line.262"
...
class Beacukaiincoming262(models.Model):
    _name = "beacukai.incoming.262"
    ...
    def _default_document_type_id(self):        
        document_type_id = self.env['beacukai.document.type'].search([('name', '=', '262')])
        if document_type_id:
    ...
    """ One to many """
    surat_keputusan_ids = fields.One2many('beacukai.surat.keputusan.262', 'reference', string='Surat Keputusan Bersama', copy=True)
    container_ids = fields.One2many('beacukai.container.262', 'reference', string='Kontainer', copy=True)
    kemasan_ids = fields.One2many('beacukai.kemasan.262', 'reference', string='Kemasan', copy=True)
    line_ids = fields.One2many('beacukai.incoming.line.262', 'reference', string='Detail Barang', copy=True)    
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('beacukai.incoming.262') or 'New'
        return super(Beacukaiincoming262, self).create(vals)

...
class BeacukaiSuratKeputusan41(models.Model):
    _name = "beacukai.surat.keputusan.41"    

    reference = fields.Many2one('beacukai.outgoing.41', 'Reference')
    name = fields.Char('Nomor Dokumen')
    jenis_dokumen = fields.Char('Jenis Dokumen')
    date = fields.Date('Tanggal')


class BeacukaiContainer41(models.Model):
    _name = "beacukai.container.41"    
...
class BeacukaiKemasan41(models.Model):
    _name = "beacukai.kemasan.41"    
...
class BeacukaioutgoingLine41(models.Model):
    _name = "beacukai.outgoing.line.41"
    _inherit = ['beacukai.outgoing.line.mixin']

    reference = fields.Many2one('beacukai.outgoing.41', 'Reference')
    jenis_kemasan = fields.Many2one('beacukai.jenis.kemasan', 'Jenis Kemasan')
    bahan_baku_import_ids = fields.One2many('beacukai.outgoing.line.bahan.baku.import.41', 'reference', 
        string='Bahan Baku Import', copy=True)
    bahan_baku_local_ids = fields.One2many('beacukai.outgoing.line.bahan.baku.local.41', 'reference', 
...
class BeacukaiOutgoingLineBahanBakuImport41(models.Model):
    _name = "beacukai.outgoing.line.bahan.baku.import.41"    

    reference = fields.Many2one('beacukai.outgoing.line.41', 'Reference')
...
class BeacukaiOutgoingLineBahanBakuLocal41(models.Model):
    _name = "beacukai.outgoing.line.bahan.baku.local.41"    
...
# class BeacukaioutgoingBahanBakuImport41(models.Model):
#     _name = "beacukai.outgoing.bahan.baku.import.41"    
...
# class BeacukaioutgoingBahanBakuLocal41(models.Model):
#     _name = "beacukai.outgoing.bahan.baku.local.41"    
...
class Beacukaioutgoing41(models.Model):
    _name = "beacukai.outgoing.41"
    ...
    def _default_document_type_id(self):        
        document_type_id = self.env['beacukai.document.type'].search([('name', '=', '41')])
        if document_type_id:
    ...
    """ One to many """
    surat_keputusan_ids = fields.One2many('beacukai.surat.keputusan.41', 'reference', string='Surat Keputusan Bersama', copy=True)
    container_ids = fields.One2many('beacukai.container.41', 'reference', string='Kontainer', copy=True)
    kemasan_ids = fields.One2many('beacukai.kemasan.41', 'reference', string='Kemasan', copy=True)
    line_ids = fields.One2many('beacukai.outgoing.line.41', 'reference', string='Detail Barang', copy=True)
    # bahan_baku_import_ids = fields.One2many('beacukai.outgoing.bahan.baku.import.41', 'reference', string='Bahan Baku Import', copy=True)
    # bahan_baku_local_ids = fields.One2many('beacukai.outgoing.bahan.baku.local.41', 'reference', string='Bahan Baku Local', copy=True)
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('beacukai.outgoing.41') or 'New'
        return super(Beacukaioutgoing41, self).create(vals)

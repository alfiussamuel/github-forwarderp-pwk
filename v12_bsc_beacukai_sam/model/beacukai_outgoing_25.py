...
class BeacukaiSuratKeputusan25(models.Model):
    _name = "beacukai.surat.keputusan.25"    

    reference = fields.Many2one('beacukai.outgoing.25', 'Reference')
    name = fields.Char('Nomor Dokumen')
    jenis_dokumen = fields.Char('Jenis Dokumen')
    date = fields.Date('Tanggal')


class BeacukaiContainer25(models.Model):
    _name = "beacukai.container.25"    
...
class BeacukaiKemasan25(models.Model):
    _name = "beacukai.kemasan.25"    
...
class BeacukaiOutgoingLine25(models.Model):
    _name = "beacukai.outgoing.line.25"
    _inherit = ['beacukai.outgoing.line.mixin']

    reference = fields.Many2one('beacukai.outgoing.25', 'Reference')
    jenis_kemasan = fields.Many2one('beacukai.jenis.kemasan', 'Jenis Kemasan')
    bahan_baku_import_ids = fields.One2many('beacukai.outgoing.line.bahan.baku.import.25', 'reference', 
        string='Bahan Baku Import', copy=True)
    bahan_baku_local_ids = fields.One2many('beacukai.outgoing.line.bahan.baku.local.25', 'reference', 
...
class BeacukaiOutgoingLineBahanBakuImport25(models.Model):
    _name = "beacukai.outgoing.line.bahan.baku.import.25"    
...
class BeacukaiOutgoingLineBahanBakuLocal25(models.Model):
    _name = "beacukai.outgoing.line.bahan.baku.local.25"    
...
# class BeacukaiOutgoingBahanBakuImport25(models.Model):
#     _name = "beacukai.outgoing.bahan.baku.import.25"    
...
# class BeacukaiOutgoingBahanBakuLocal25(models.Model):
#     _name = "beacukai.outgoing.bahan.baku.local.25"    
...
class BeacukaiOutgoing25(models.Model):
    _name = "beacukai.outgoing.25"
    ...
    def _default_document_type_id(self):        
        document_type_id = self.env['beacukai.document.type'].search([('name', '=', '25')])
        if document_type_id:
    ...
    contract_date = fields.Char('Tgl Kontrak')
    ...
    """ One to many """
    surat_keputusan_ids = fields.One2many('beacukai.surat.keputusan.25', 'reference',
                                          string='Surat Keputusan Bersama', copy=True)
    container_ids = fields.One2many('beacukai.container.25', 'reference', string='Kontainer', copy=True)
    kemasan_ids = fields.One2many('beacukai.kemasan.25', 'reference', string='Kemasan', copy=True)
    line_ids = fields.One2many('beacukai.outgoing.line.25', 'reference', string='Detail Barang', copy=True)
    # bahan_baku_import_ids = fields.One2many('beacukai.outgoing.bahan.baku.import.25', 'reference',
    #                                         string='Bahan Baku Import', copy=True)
    # bahan_baku_local_ids = fields.One2many('beacukai.outgoing.bahan.baku.local.25', 'reference',
    #                                        string='Bahan Baku Local', copy=True)
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('beacukai.outgoing.25') or 'New'
        return super(BeacukaiOutgoing25, self).create(vals)

...
class RpbBahanBakuReportXls(models.AbstractModel):
    _name = 'report.v12_pwk.rpb_bahan_baku_report_xls.xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, lines):        
        ...
        if lines.bahan_baku_ids:
            veneer_ids = self.env['pwk.rpb.bahan.baku'].search([
                ('reference', '=', lines.id),
                ('goods_type', '=', 'Veneer')
            ])

            faceback_ids = self.env['pwk.rpb.bahan.baku'].search([
                ('reference', '=', lines.id),
                ('goods_type', '=', 'Faceback')
            ])

            mdf_ids = self.env['pwk.rpb.bahan.baku'].search([
                ('reference', '=', lines.id),
                ('goods_type', '=', 'MDF')
            ])

            barecore_ids = self.env['pwk.rpb.bahan.baku'].search([
                ('reference', '=', lines.id),
                ('goods_type', '=', 'Barecore')
            ])

...
class RpmReportXls(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):        
        ...
        sheet.write(row + 1, 5, lines.total_produksi, formatHeaderDetailRightFour)
        sheet.write(row + 2, 5, lines.total_blockboard, formatHeaderDetailRightFour)
        sheet.write(row + 3, 5, lines.total_plywood, formatHeaderDetailRightFour)
        sheet.write(row + 4, 5, lines.total_lvl, formatHeaderDetailRightFour)
        sheet.write(row + 5, 5, lines.target_per_hari, formatHeaderDetailRightFour)

        sheet.write(row + 2, 6, str(lines.total_blockboard_percent) + '%', formatHeaderDetailRight)
        sheet.write(row + 3, 6, str(lines.total_plywood_percent) + '%', formatHeaderDetailRight)
        sheet.write(row + 4, 6, str(lines.total_lvl_percent) + '%', formatHeaderDetailRight)
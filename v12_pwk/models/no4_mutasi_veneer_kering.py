...
class PwkMutasiVeneerKeringLine(models.Model):
    ...
    supp_stock_masuk_pcs = fields.Float(compute="_get_stock_masuk", string='Stok Masuk Supp')
        ...
        for res in self:
            ...
            if res.line_ids:
                for line in res.line_ids:
                    new_product_name = 'Veneer Kering ' + line.product_id.jenis_kayu.name + '  ' + str(line.product_id.tebal) + ' x ' + str(line.product_id.lebar) + ' x ' + str(line.product_id.panjang) + ' Grade ' + line.product_id.grade.name.name
                    ...
                    line.write({
                        'new_product_id': new_product_id.id
                    })
